...
const Navbar = () => {
    ...
    const AuthButtons = !localStorage.getItem('auth_token') ? (
        ...
        <>
            <li className="nav-item">
                <button type="button" onClick={logoutSubmit} className="nav-link btn btn-outline-danger btn-sm text-white">Logout</button>
            </li>
        </>
    );

    return (
        <nav className="sb-topnav navbar navbar-expand navbar-dark bg-dark" style={{height: "80px", fontSize: "1.2rem"}}>
